...
bool operator<(const StringInt &lhs, const StringInt &rhs)
{
    ...
    if (((lhs.sign == '+') &&
         (rhs.sign == '+') &&
         (abs_comparator_result == StringInt::Comparator_result::LESS)) ||
        ((lhs.sign == '-') &&
         (rhs.sign == '-') &&
         (abs_comparator_result == StringInt::Comparator_result::GREATER)))
    {
        return true;
    }
    return false;
...
{
    std::string result;
    size_t max_length = std::max(one.length(), another.length());
    int overflow = 0;
