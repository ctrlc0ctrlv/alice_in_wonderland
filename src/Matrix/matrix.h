...
public:
    ...
    Matrix(const Matrix &m) : 
        N(m.N), M(m.M), eps(m.eps), data(m.data), transp(m.transp) {}
    ...
    Matrix<T> operator+(const Matrix<T> &v) const {
        if (v.getN() != this->N || v.getM() != this->M) { throw DimensionsMismatch(); }
        Matrix<T> res(*this);
    ...
    Matrix<T> operator-(const Matrix<T> &m) const {
        if (m.getN() != this->N || m.getM() != this->M) { throw DimensionsMismatch(); }
        Matrix<T> res(*this);
    ...
    Matrix<T> operator*(const Matrix<T> &m) const {
        ...
        for (auto [key, val] : data) {
            auto [i, j] = key;
            if (transp) { std::swap(i, j); }
            for (auto [key2, val2] : m.data) {
                auto [i2, j2] = key2;
                if (m.transp) { std::swap(i2, j2); }
    ...
    bool operator==(const Matrix<T> &m) const {
        if (N != m.N || M != m.M) { return false; }
        for (auto [key, val] : data) {
            auto [i, j] = key;
            if (transp) { std::swap(i, j); }
            if (val != m.at(i, j)) { return false; }
        }
        for (auto [key, val] : m.data) {
            auto [i, j] = key;
            if (m.transp) { std::swap(i, j); }
            if (val != this->at(i, j)) { return false; }
        }
        return true;
    }
